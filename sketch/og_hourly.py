...
def chr_counter():
    yield from (chr(n + 97) for n in count())
...
class Task:
    ...
    def get_print_string(self, prefix="", letters=False, skip_finished=False):
        if self.finished:
            return ""
        ...
        text = "\n".join(self.text)
        subtasks = (
            [subtask for subtask in self.subtasks if not subtask.finished]
            if skip_finished
            else self.subtasks
        )
        counter = chr_counter if letters else count
        return f"{prefix} {inactive}{checkbox} - {text}" + "".join(
            [
                f"\n  {subtask.get_print_string(prefix=prefix+str(i))}"
                for i, subtask in zip(counter, subtasks)
            ]
        )
...
def get_todos():
    def todo_key(todo_str):
        finished = todo_str[1] != " "
        pound_loc = todo_str.find("#")
        if pound_loc > -1:
            timestamp = datetime.datetime.strptime(
                todo_str[pound_loc + 2 : pound_loc + 7], "%H:%M"
            ).time()
        else:
            timestamp = datetime.datetime.max.time()
        task = todo_str[todo_str.find("]") + 2 :]
        if not task.split()[0].isupper():
            task = ""
        key = (task, finished, timestamp)
        return key

    todos = sorted(
        [l for l in get_last_day() if l.strip().startswith(("["))], key=todo_key
    )
    return todos
...
def todo_loop():
    while True:
        ...
        for i, todo in enumerate(get_todos()):
            sublist = todo[todo.find("]") + 2 :].split()[0]
            if sublist.isupper() and cur_sublist != sublist:
                print("*******")
                cur_sublist = sublist
            print(i, todo, end="")
        while True:
...
def newday(note):
    ...
    log = (
        datetime.datetime.now().strftime("---%m/%d/%Y (%A)---")
        + f" {note}\n"
