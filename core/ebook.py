...
class Chapter(tp.NamedTuple):
    ...
    def to_epub(self) -> epub.EpubHtml:
        epub.EpubHtml(title=self.title, file_name=f'{self.num}.xhtml', lang='jp', content=self.text)
...
def get_chapters(novel_id: str, num_chapters: int) -> list[Chapter]:
    return [response_to_text(response, num=i) for i, response in enumerate(gen_responses(novel_id, num_chapters), 1)]
